 ...
 * Feishu, Webhook and Stdin round-trip for real (HTTP in + reply out). Slack has a real inbound but a
 * stubbed outbound (it cannot post a reply) — it cannot hold a full conversation, so it is flagged
 * {@code functional=false}. Callers (the seed config, {@code /channel}) use {@link #isFunctional()}
 * to mark it clearly and refuse to enable/test it, rather than pretending it connects.
 */
public enum ChannelType {
     ...
     * the reply-less Slack outbound — a placeholder, not a usable channel, so callers should
     * mark/refuse it rather than pretend it connects.
     */
