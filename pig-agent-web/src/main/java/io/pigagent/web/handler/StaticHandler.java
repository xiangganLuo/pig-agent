...
public final class StaticHandler implements HttpHandler {
    ...
    public void handle(HttpExchange ex) throws IOException {
        try {
            ...
            // Serve only the known static asset types (html/js/css). Even though getResourceAsStream is
            // bounded to the classpath, an explicit allowlist keeps unrelated resources under /web/
            // unreachable and guards against future filesystem-serving drift.
            if (!isServableAsset(path)) {
    ...
    private static boolean isServableAsset(String path) {
