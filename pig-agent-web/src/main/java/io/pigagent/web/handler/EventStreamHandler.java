 ...
 * driving the live console (session stream, run monitor). One server thread is held open per
 * connected client — fine for a single-user local console.
 *
 ...
 * disposed and the exchange closed ("no subscribers = zero cost" per the facade contract).
 */
public final class EventStreamHandler implements HttpHandler {
    ...
    public void handle(HttpExchange ex) throws IOException {
        if (!"GET".equals(ex.getRequestMethod())) {
            // SSE is a GET resource: reject other verbs instead of committing an open stream + thread.
