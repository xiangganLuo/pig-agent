 ...
 *
 * <p>Routes: {@code /api/agents…} (REST → façade), {@code /api/events} (SSE ← {@code subscribeEvents}),
 * {@code /} (static frontend). Construct with {@code (kernel, host, port)}; call {@link #start()} and
 * {@link #stop()}. Port {@code 0} picks an ephemeral port — read it back via {@link #boundPort()}.
 */
public final class WebConsole {
    ...
    public WebConsole(AgentKernel kernel, String host, int port) {
    ...
    public synchronized void start() throws IOException {
        ...
        s.createContext("/api/agents", new AgentApiHandler(kernel, json));
        s.createContext("/api/events", new EventStreamHandler(kernel, json));
