...
**Local Web console (`pig-agent-web`, `web-visualization`).** An optional second frontend, built as another `AgentKernel` adapter with **zero kernel logic** (D1). It runs on the JDK's built-in `com.sun.net.httpserver` — **no external web framework** (light, offline-safe; the design's Open Question is resolved toward jdk.httpserver over Javalin). `WebConsole(kernel, host, port)` exposes REST at `/api/agents…` (`AgentApiHandler` → `list/get/create/update/delete/use/runNow`), an SSE stream at `/api/events` (`EventStreamHandler` subscribes to `subscribeEvents()` and pushes `KernelEvent`s as `data:` frames, with a `retry:` hint for auto-reconnect), and a static MVP frontend at `/` (`StaticHandler` serves `resources/web/{index.html,app.js,style.css}` — plain HTML/JS, no build step, D5). **Security floor (D3):** binds a loopback host only (`web.host` default `127.0.0.1`), single-user, no auth, no DB. Gated by `PigAgentConfig.WebConfig` (`web.enabled` default **false**); when on, `PigAgentCli.main` builds one `WebConsole` sharing the CLI's `agentKernel` and stops it in the shutdown hook. Port `0` picks an ephemeral port (`boundPort()`).
