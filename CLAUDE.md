...
- New tool: a class with `@Tool`/`@ToolParam` methods → `toolkit.registration().tool(new MyTool()).apply()`. Return the tool's normal output on success and `ToolErrors.message(reason)` on failure (canonical `{"error"}`, credential-redacted); don't throw as the model-facing error path (the dispatch guard is a net, not the primary contract).
