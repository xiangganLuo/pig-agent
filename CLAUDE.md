...
| `pig-agent-tools` | `@Tool`-annotated built-in tools + SPI auto-registration | `ShellTools`, `FileSystemTools`, `SmartWebFetchTool`, `BraveWebSearchTool`, `TaskTool`, `CheckListTool`, `SkillsTool`, `ToolDiscovery` (Lucene), `spi/{ToolProvider, ToolContext, ToolRegistrar}` |
