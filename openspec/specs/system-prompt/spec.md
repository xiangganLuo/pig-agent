...
## Purpose
为编码/终端 agent 播种一份详尽、结构化、英文的生产级默认系统提示词（`WorkspaceManager.defaultAgentMd()`），准确映射最终能力集的真实工具/插件/技能与「人在回路」权限沙箱模型（读只读放行、可变/执行/网络/MCP 管理类可能需确认或被拒、凭据文件禁区、密钥不回显、被拒/不可用优雅降级），并要求按用户的语言应答；`AgentBootstrap` 追加的固定 `TOOL_GUIDANCE` 与之对齐并固化「永远成立」的硬约束（即便用户重写 `AGENT.md` 仍生效）。默认提示词经 `createIfAbsent` 播种——仅新工作区，现有 `AGENT.md` 逐字保留——仍是用户可编辑模板，且每次运行 byte-stable（前缀缓存友好）。
## Requirements
