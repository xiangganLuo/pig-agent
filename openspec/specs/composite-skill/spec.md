...
## Purpose
把技能从单份 `SKILL.md` 升级为**复合能力包（目录 bundle）**，镜像 DeerFlow 的 composite Skill：SKILL.md 支持可选 YAML front-matter 元数据（`SkillMetadata` 值类型 + `SkillManifestParser` 策略，容错解析，缺失/部分/畸形均降级派生，7 个内置技能补 front-matter 后向后兼容）；技能目录可携带支持文件并经 `loadSkill` 有界浮现（小文本内联、二进制/超限只列名，`SkillLimits` 值对象统一上限）；加载**渐进**——`listSkills` 仅读元数据（省 token）、`loadSkill` 按需读正文；`WorkspaceSkillSource` 对用户投放的技能做**路径遍历/符号链接逃逸/尺寸加固**（`SkillSecurity` 纯静态 real-path 守卫，镜像 `SsrfGuard`），畸形/超限/逃逸技能容错跳过。`SkillsTool` 的 `@Tool` 名/签名/返回语义逐字不变（无模型面回归）。
## Requirements
