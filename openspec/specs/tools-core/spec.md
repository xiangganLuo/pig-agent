...
## Purpose
按设计属性（而非功能）划分工具的归属：`pig-agent-tools` 只承载「高可用、零外部配置、无重依赖」的核心工具 + 工具框架；需外部配置/联网/非基本操作的工具下沉为内置插件（`pig-agent-plugin-builtin`）；核心模块不含死代码与重依赖。目标是让核心工具模块小而稳、职责清晰，抽取的工具经既有 `Plugin` SPI 开箱注册且行为逐字保留（可用性门控、SSRF、风险分级）。
## Requirements
