...
## Purpose
工程化质量门：用 jacoco 度量单元测试覆盖率，并以**每模块棘轮基线**（floor = floor(current)−0.02，绑定 `verify` 阶段的 `jacoco:check`）锁死覆盖率只进不退且今日必过；用 failsafe 让 `*IT` 集成测试**可选**进入 `mvn verify`（默认跳过，`-Pit` 启用），保证无凭据的默认 `mvn verify` 仍绿。最高风险的运行时编排器具备离线、确定性、零网络单测；门与度量所依赖的测试套件在并行构建下确定通过、无 flaky。作为 CI/本地可持续演进的自动门。
