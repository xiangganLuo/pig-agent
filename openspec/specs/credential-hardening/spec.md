# credential-hardening Specification

## Purpose
凭据的存取卫生：录入时终端掩码（不回显、不留 scrollback）与落盘时文件权限收敛（`0600`）。与 `tool-sandbox` 同属生产就绪度审查 P0 的凭据保护，覆盖 API key / MCP header/env token 的录入与持久化两端。
