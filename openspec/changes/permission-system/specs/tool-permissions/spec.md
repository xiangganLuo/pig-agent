...
### Requirement: 非交互渠道兜底
渠道（Telegram/Discord 等无终端）回合 MUST 使用 `permissions.channel-mode`（默认 `auto`）而非交互模式。当某决策需要人工确认但无可用 confirmer 时，系统 MUST fail-closed 拒绝并回传清晰说明，除非 `channel-mode=bypass`。

#### Scenario: 渠道 auto 下 shell 被拒
- **WHEN** `channel-mode=auto`、来自 Telegram 的回合触发 shell 执行（EXEC 在 auto 下需确认）
- **THEN** 因无终端可确认，请求被 fail-closed 拒绝并说明原因，不执行
