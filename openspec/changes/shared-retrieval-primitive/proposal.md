...
- **ranker 核心上提到 `io.pigagent.core.search`**：`Bm25Index`、`HybridRanker`、`Tokenizer`（CJK unigram+bigram，保持通用直接复用）迁入新包。**`HybridRanker.rank(...)` 已是纯静态泛型函数（`Map<String,Double>`→ranked），完全不动**（已核实）。**范围收窄**：向量/嵌入层（`VectorStore`/`InMemoryVectorStore`/`Embedder`/`DeterministicEmbedder`/`OpenAiCompatibleEmbedder`/`Vectors`）当前仅记忆线消费，**留在 `memory.search`**——保证记忆门面/工具单测逐字不改；向量层上提待真有跨线消费方时按 YAGNI 再做（不影响 ranker 核心契约）。
- **记忆域类型留在 `io.pigagent.core.memory.search`**：`MemoryDocument`、`MemoryCorpusLoader`（语料来源 `MEMORY.md`/`memory/*.md`/`USER.md` + chunk 策略）、`MemorySearchIndex`（记忆检索门面）、`MemorySearchConfig` + 上述向量/嵌入层——它们是记忆线私有编排，import 变更消费新包的 ranker 核心，对外契约不变。
...
- **代码（纯重构，零行为变化）**：
  - `pig-agent-core` 新增包 `io.pigagent.core.search`：新增 `SearchDocument` 接口；从 `io.pigagent.core.memory.search` 迁入 ranker 核心 `Bm25Index`/`HybridRanker`/`Tokenizer`。
  - `pig-agent-core` `io.pigagent.core.memory.search` 保留 `MemoryDocument`（新增 `implements SearchDocument`）/`MemoryCorpusLoader`/`MemorySearchIndex`/`MemorySearchConfig` + 向量/嵌入层（`VectorStore`/`InMemoryVectorStore`/`Embedder`/`DeterministicEmbedder`/`OpenAiCompatibleEmbedder`/`Vectors`，仅记忆线消费，暂不上提），import 更新指向新包（`MemorySearchIndex` 取 `Bm25Index`/`HybridRanker`；`DeterministicEmbedder` 取 `Tokenizer`）。
