...
- **D3 — 包边界：`io.pigagent.core.search` 收 ranker 核心，`io.pigagent.core.memory.search` 留记忆域 + 向量/嵌入层**。迁入新包（ranker 核心）：`SearchDocument`(新)、`Bm25Index`、`HybridRanker`、`Tokenizer`。留原包：`MemoryDocument`（+`implements SearchDocument`）、`MemoryCorpusLoader`、`MemorySearchIndex`、`MemorySearchConfig`，**以及向量/嵌入层** `VectorStore`/`InMemoryVectorStore`/`Embedder`/`DeterministicEmbedder`/`OpenAiCompatibleEmbedder`/`Vectors`。**范围收窄理由（执行门确认）**：`MemorySearchIndexTest`/`HybridMemorySearchToolTest` 直接 `new InMemoryVectorStore()`/`new DeterministicEmbedder()`——把这些类留在 `memory.search` 才能让这两个门面/工具测试**逐字不改**（硬约束）；且 T2/S2 的首个消费面是 BM25 排序，ranker 核心已足够。向量/嵌入层的上提按 YAGNI 待真有跨线消费方时再做，届时 ranker 核心契约不变。这条边界让 T2/S2/M-* 只 import `core.search` 即可做 BM25 排序，不牵扯记忆域。
